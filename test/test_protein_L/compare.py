...
ax1.plot(merged.amp_x,merged.amp_y,"o",label=f"slope={fit.params['slope'].value:.2f}")
ax1.plot([2.5e8,6.5e8],[2.5e8,6.5e8],"k--",alpha=0.75)

mod = LinearModel()
fit = mod.fit(merged.X_PPM,x=merged.center_x_ppm)
ax2.plot(merged.center_x_ppm, merged.X_PPM,"o",label=f"slope={fit.params['slope'].value:.2f}")
ax2.plot([7,10.5],[7,10.5],"k--",alpha=0.75)

mod = LinearModel()
fit = mod.fit(merged.Y_PPM,x=merged.center_y_ppm)
ax3.plot(merged.center_y_ppm, merged.Y_PPM,"o",label=f"slope={fit.params['slope'].value:.2f}")
...
ax6.text(.4,.5,"Fit using Gaussian lineshape",ha="center",va="center",transform=ax6.transAxes)
